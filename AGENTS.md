...
- Dedicated Dealyard browser root: `~/.cache/dealyard/browser/chrome-user-data/`
...
- Treat host-level process control as hazardous. Dealyard must never mass-match, mass-kill, or desktop-drive the shared machine just to recover repo state.
...
- This machine is a **multi-repo** environment. Do not assume any existing Chrome / Chromium window, tab, profile, or debug listener belongs to Dealyard.
- The long-term Dealyard browser contract now uses a dedicated Chrome root at `~/.cache/dealyard/browser/chrome-user-data/`.
- Before browser work, inspect the current machine state and decide whether the target instance is provably Dealyard-owned or not.
...
- Only attach to a browser instance when you can prove it is Dealyard-owned:
  - repo-owned `CHROME_USER_DATA_DIR`
  - explicit Dealyard profile contract
  - or a Dealyard-created throwaway debug lane for the current turn
- A real Dealyard Chrome profile contract now requires all three values together:
...
- Before running the one-time migration into the dedicated Dealyard root, fully quit any real Google Chrome process still using the default macOS Google Chrome user-data root.
- The preferred long-term mode is one dedicated Dealyard Chrome instance plus CDP attach:
...
- The canonical Dealyard browser lane now also includes a human-facing identity anchor at `.runtime-cache/browser-identity/index.html`.
...
- Do not treat an existing shared Chrome window, a random open tab, or an unknown reachable listener as Dealyard's browser just because it is there.
...
- For real authenticated profile / listener checks, 1 to 2 fresh attempts are enough. If those attempts already prove the required Dealyard-owned session/listener is absent, stop, record it as an external blocker in the closeout ledger, and do not brute-force more browser launches.
...
- If the current turn opens tabs or windows for Dealyard, this repo owns the responsibility to close those same temporary tabs/windows before closeout.

## Resource Hygiene

- Only clean Dealyard-owned rebuildables, caches, temp directories, browser profiles, Docker artifacts, and operator byproducts.
...
- `.serena/` is a local MCP/tool cache namespace. Keep it ignored, but do not count it as Dealyard-owned cache and do not include it in Dealyard cleanup/budget ledgers.
- Shared-layer caches such as `~/.cache/uv`, `~/.cache/pre-commit`, `~/.cache/node/corepack`, `~/Library/Caches/ms-playwright`, and `~/.npm` remain audit-only and must not be auto-cleaned by Dealyard.
...
- Do **not** run global cleanup like `docker system prune -a`, and do not delete containers / images / volumes you cannot prove belong to Dealyard.
...
- If a Docker container/image/volume or browser temp/profile cannot be proved to belong to Dealyard, record it as `ownership unknown` and leave it alone.
...
- That means no ad-hoc posting, messaging, account/profile edits, LinkedIn/Reddit/Discourse/Xiaohongshu activity, browser/bookmark/history changes, Messages/contact changes, or third-party platform writes that are not strictly required for Dealyard Git/GitHub closeout.
