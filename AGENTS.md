...
- `dealwatcherer` is the only live name
- No `tracker` aliases
...
- Legacy import source: `.legacy-runtime/data/dealwatcherer.db`
- Runtime artifacts: `.runtime-cache/`
- Repo-local operator artifacts: `.runtime-cache/operator/`
- Repo-owned external cache root: `~/.cache/dealwatcherer/`
- Dedicated Dealwatcher browser root: `~/.cache/dealwatcherer/browser/chrome-user-data/`
- Product runtime maintenance entrypoint: `python -m dealwatcherer maintenance --dry-run|--apply`
...
- `src/dealwatcherer/core/`, `src/dealwatcherer/stores/`: engine
- `src/dealwatcherer/persistence/`, `src/dealwatcherer/application/`, `src/dealwatcherer/api/`, `src/dealwatcherer/worker/`: product
- `frontend/`: WebUI
...
- Treat host-level process control as hazardous. Dealwatcher must never mass-match, mass-kill, or desktop-drive the shared machine just to recover repo state.
...
- Only signal a repo-recorded positive child PID that Dealwatcher itself spawned.
...
- Browser/profile cleanup must stay on repo-owned entrypoints such as `./scripts/launch_dealwatcherer_chrome.sh`, `python3 scripts/open_dealwatcherer_account_pages.py`, `python3 scripts/cleanup_local_rebuildables.py`, and `python -m dealwatcherer maintenance --dry-run|--apply`; if ownership is ambiguous, refuse and report it instead of guessing.
...
- This machine is a **multi-repo** environment. Do not assume any existing Chrome / Chromium window, tab, profile, or debug listener belongs to Dealwatcher.
- The long-term Dealwatcher browser contract now uses a dedicated Chrome root at `~/.cache/dealwatcherer/browser/chrome-user-data/`.
- Before browser work, inspect the current machine state and decide whether the target instance is provably Dealwatcher-owned or not.
...
- Only attach to a browser instance when you can prove it is Dealwatcher-owned:
  - repo-owned `CHROME_USER_DATA_DIR`
  - explicit Dealwatcher profile contract
  - or a Dealwatcher-created throwaway debug lane for the current turn
- A real Dealwatcher Chrome profile contract now requires all three values together:
  - `CHROME_USER_DATA_DIR`
  - `CHROME_PROFILE_NAME`
  - `CHROME_PROFILE_DIRECTORY`
- The old shared-root contract is deprecated for `dealwatcherer`; do not point `CHROME_USER_DATA_DIR` at the default macOS Google Chrome user-data root when `CHROME_PROFILE_NAME=dealwatcherer`.
- Before running the one-time migration into the dedicated Dealwatcher root, fully quit any real Google Chrome process still using the default macOS Google Chrome user-data root.
- The preferred long-term mode is one dedicated Dealwatcher Chrome instance plus CDP attach:
...
- The canonical Dealwatcher browser lane now also includes a human-facing identity anchor at `.runtime-cache/browser-identity/index.html`.
...
- Do not treat an existing shared Chrome window, a random open tab, or an unknown reachable listener as Dealwatcher's browser just because it is there.
...
- For real authenticated profile / listener checks, 1 to 2 fresh attempts are enough. If those attempts already prove the required Dealwatcher-owned session/listener is absent, stop, record it as an external blocker in the closeout ledger, and do not brute-force more browser launches.
...
- If the current turn opens tabs or windows for Dealwatcher, this repo owns the responsibility to close those same temporary tabs/windows before closeout.

## Resource Hygiene

- Only clean Dealwatcher-owned rebuildables, caches, temp directories, browser profiles, Docker artifacts, and operator byproducts.
- Repo-owned lightweight cache budget now targets `.runtime-cache/**` plus `~/.cache/dealwatcherer/**`.
- `~/.cache/dealwatcherer/browser/chrome-user-data/**` is a persistent dedicated browser workspace. Exclude it from TTL cleanup, cache budget reclamation, and rebuildable cleanup.
...
- `.serena/` is a local MCP/tool cache namespace. Keep it ignored, but do not count it as Dealwatcher-owned cache and do not include it in Dealwatcher cleanup/budget ledgers.
- Shared-layer caches such as `~/.cache/uv`, `~/.cache/pre-commit`, `~/.cache/node/corepack`, `~/Library/Caches/ms-playwright`, and `~/.npm` remain audit-only and must not be auto-cleaned by Dealwatcher.
- Prefer repo-owned cleanup entrypoints before ad-hoc shell deletion:
  ...
  - `python -m dealwatcherer maintenance --dry-run|--apply`
- When browser/Docker/cache ownership is part of the task, load the repo-local skill `resource-hygiene-and-browser-isolation`.
- Do **not** run global cleanup like `docker system prune -a`, and do not delete containers / images / volumes you cannot prove belong to Dealwatcher.
...
- If a Docker container/image/volume or browser temp/profile cannot be proved to belong to Dealwatcher, record it as `ownership unknown` and leave it alone.
...
- That means no ad-hoc posting, messaging, account/profile edits, LinkedIn/Reddit/Discourse/Xiaohongshu activity, browser/bookmark/history changes, Messages/contact changes, or third-party platform writes that are not strictly required for Dealwatcher Git/GitHub closeout.
