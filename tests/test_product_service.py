...
import dealwatcherer.persistence.store_bindings as store_bindings_module
...
from dealwatcherer.persistence.session import create_session_factory, get_session_factory, init_product_database
...
async def test_product_service_watch_group_detail_includes_fake_ai_explain(tmp_path, monkeypatch) -> None:
    ...
    service = ProductService(
        ...
        settings=Settings(
            ...
            USE_LLM=True,
            AI_PROVIDER="fake",
            AI_MODEL="dealwatcherer-fake-v1",
...
async def test_product_service_builder_starter_pack_reports_current_builder_contract(tmp_path) -> None:
    ...
    assert payload["launch_contract"]["mcp_inventory"].endswith("dealwatcherer.mcp list-tools --json")
...
async def test_product_service_store_onboarding_cockpit_reports_real_store_truth(tmp_path) -> None:
    ...
    assert payload["onboarding_contract"]["required_files"][0]["path_template"] == "src/dealwatcherer/stores/<store>/adapter.py"
