...
SAFETY_RULES = (
    ...
    SafetyRule(
        ...
        message="direct `process.kill(...)` is forbidden; Dealwatcher must not emit raw host signals from repo code.",
    ),
    SafetyRule(
        name="os_kill",
        pattern=re.compile(r"\bos\.kill\s*\("),
        message="direct `os.kill(...)` is forbidden; Dealwatcher must not emit raw host signals from repo code.",
    ),
    SafetyRule(
        name="osascript",
        pattern=re.compile(r"\bosascript\b"),
        message="`osascript` is forbidden in executable Dealwatcher paths; browser control must stay on repo-owned contracts.",
    ),
    SafetyRule(
        name="system_events",
        pattern=re.compile(r"System Events"),
        message="desktop-wide `System Events` automation is forbidden in Dealwatcher worker/test/runtime paths.",
    ),
    SafetyRule(
    ...
    SafetyRule(
        ...
        message="AppleEvent-driven app/session control is forbidden in Dealwatcher executable paths.",
    ),
)
