...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Report whether each Dealwatcher browser lane target currently looks like homepage-logged-in, account-page-logged-in, or reauth-required."
    )
    parser.add_argument(
        "--env-file",
...
def select_context_for_target(contexts: list[Any], target: BrowserLaneTargetSpec) -> Any:
    if not contexts:
        raise ValueError("Dealwatcher browser login-state report requires at least one browser context.")
...
def render_text(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "Dealwatcher browser login-state report",
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(
            f"Dealwatcher browser login-state report failed: could not attach to {contract.cdp_url} ({exc}). Launch or reuse the dedicated browser lane first."
        ) from exc
