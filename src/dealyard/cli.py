...
from dealwatcherer.api.deps import get_product_service, prepare_product_runtime
from dealwatcherer.builder_contract import list_client_ids
from dealwatcherer.core.models import RunStats
from dealwatcherer.core.pipeline import MonitoringPipeline
from dealwatcherer.infra.config import load_enabled_stores_from_yaml, settings
from dealwatcherer.infra.browser_debug import diagnose_browser_debug, probe_browser_debug, write_browser_support_bundle
...
from dealwatcherer.persistence.session import session_scope
from dealwatcherer.server import main as server_main
from dealwatcherer.stores import STORE_REGISTRY
from dealwatcherer.worker.main import main as worker_main, run_once as worker_run_once
...
def _print_main_help(stream: object) -> None:
    print("Usage: python -m dealwatcherer <command> [...].", file=stream)
    print("", file=stream)
...
def _parse_legacy_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dealwatcher legacy SQLite bridge importer (deprecated)"
    )
    parser.add_argument(
        "--store",
        action="append",
...
def _parse_legacy_maintenance_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dealwatcher legacy SQLite bridge maintenance (deprecated)"
    )
    return parser.parse_args(argv)


def _parse_bootstrap_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dealwatcher owner bootstrap")
...
def _parse_maintenance_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dealwatcher product runtime maintenance"
    )
...
def _parse_builder_starter_pack_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dealwatcher builder starter pack")
    parser.add_argument(
        "--json",
        action="store_true",
...
def _parse_builder_client_config_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dealwatcher builder client config export")
    parser.add_argument(
        "client",
        nargs="?",
...
async def _run_builder_starter_pack(argv: list[str]) -> int:
    ...
    if args.json:
        ...
        print("Dealwatcher builder starter pack")
...
async def _run_builder_client_config(argv: list[str]) -> int:
    ...
    if args.json:
        ...
        if args.all:
            print("Dealwatcher builder client config bundle")
...
async def _run_probe_live(argv: list[str]) -> int:
    _parse_browser_debug_args(argv, description="Dealwatcher maintainer browser debug probe")
...
async def _run_diagnose_live(argv: list[str]) -> int:
    _parse_browser_debug_args(argv, description="Dealwatcher maintainer browser debug diagnosis")
...
async def _run_support_bundle(argv: list[str]) -> int:
    _parse_browser_debug_args(argv, description="Dealwatcher maintainer browser debug support bundle")
...
def main() -> None:
    ...
    print(f"Unknown Dealwatcher command: {command}.", file=sys.stderr)
