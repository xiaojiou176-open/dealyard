...
from dealwatcherer.api.deps import get_product_service, prepare_product_runtime, shutdown_product_runtime
...
class DealwatcherMcpRuntime(ReadonlyProductMcpBridge):
...
def create_mcp_server(bridge: ReadonlyProductMcpBridge | None = None) -> FastMCP:
    readonly_bridge = bridge or DealwatcherMcpRuntime()
    server = FastMCP(
        name="dealwatcherer",
        instructions=(
            "Read-only Dealwatcher MCP server. It exposes compare, watch, runtime, notification, "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Dealwatcher read-only MCP server")
    subcommands = parser.add_subparsers(dest="command", required=True)

    list_tools_parser = subcommands.add_parser("list-tools", help="Print the registered Dealwatcher MCP tools.")
    ...
    client_starters_parser = subcommands.add_parser(
        "client-starters",
        help="Print repo-owned Dealwatcher starter metadata for local MCP/API clients.",
    )
    ...
    client_config_parser = subcommands.add_parser(
        "client-config",
        help="Print one repo-owned Dealwatcher client config export.",
    )
    ...
    serve_parser = subcommands.add_parser("serve", help="Run the Dealwatcher MCP server.")
