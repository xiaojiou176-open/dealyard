...
For Dealwatcher, the cleanest current translation is to pair that official wrapper with the repo-owned HTTP-facing transport:
...
```bash
codex mcp add dealwatcherer --url http://127.0.0.1:8000/mcp
```
...
This repo verifies the Dealwatcher side of the handoff:
...
That makes the wrapper syntax itself official, while Dealwatcher still keeps the product promise read-only-first and local-first.
